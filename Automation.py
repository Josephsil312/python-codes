...
web = web.Chrome(r'C:\Program Files (x86)\chromedriver')  # system path of chromedriver
web.get(
    r'https://docs.google.com/forms/d/e/1FAIpQLSdjUSiRPAD3jUqsUGB2ATcyMfOrFWt8Y3Jsoyuxsd19eniZiQ/viewform?usp=sf_link')  # this is url link of attendence form
...
def Monday():
    ...
    for i in range(0, 4):
        # Name
        fname = web.find_element_by_xpath(
            '//*[@id="mG61Hd"]/div[2]/div/div[2]/div[1]/div/div/div[2]/div/div[1]/div/div[1]/input')
        fname.send_keys(name)

        # Email ID

        femail = web.find_element_by_xpath(
        ...
        dropdown.click()
        time.sleep(0.80)
        item = web.find_element_by_css_selector(
            '.exportSelectPopup [data-value="{hey}"]'.format(hey=monday_subjectlst[i]))
        item.click()

        # Date

        date = finaltime
        fdate = web.find_element_by_xpath(
        ...
        hr = tymlst[i][0]
        fhr = web.find_element_by_xpath(
            '//*[@id="mG61Hd"]/div[2]/div/div[2]/div[6]/div/div/div[2]/div/div[1]/div[2]/div[1]/div/div[1]/input')
        fhr.send_keys(hr)

        mi = tymlst[i][1]
        fmi = web.find_element_by_xpath(
            '//*[@id="mG61Hd"]/div[2]/div/div[2]/div[6]/div/div/div[2]/div/div[3]/div/div[1]/div/div[1]/input')
        fmi.send_keys(mi)
...
def Tuesday():
    ...
    for i in range(0, 4):
        ...
        time.sleep(0.80)
        item = web.find_element_by_css_selector(
            '.exportSelectPopup [data-value="{hey}"]'.format(hey=Tuesday_subjectlst[i]))
        item.click()
...
def Wednesday():
    ...
    for i in range(0, 4):
        ...
        time.sleep(0.80)
        item = web.find_element_by_css_selector(
            '.exportSelectPopup [data-value="{hey}"]'.format(hey=Wednesday_subjectlst[i]))
        item.click()
...
def Thursday():
    ...
    for i in range(0, 4):
        ...
        time.sleep(0.80)
        item = web.find_element_by_css_selector(
            '.exportSelectPopup [data-value="{hey}"]'.format(hey=Thursday_subjectlst[i]))
        item.click()
...
def Friday():
    ...
    for i in range(0, 4):
        ...
        time.sleep(0.80)
        item = web.find_element_by_css_selector(
            '.exportSelectPopup [data-value="{hey}"]'.format(hey=Friday_subjectlst[i]))
        item.click()
...
def Saturday():
    ...
    for i in range(0, 4):
        ...
        time.sleep(0.80)
        item = web.find_element_by_css_selector(
            '.exportSelectPopup [data-value="{hey}"]'.format(hey=Saturday_subjectlst[i]))
        item.click()
